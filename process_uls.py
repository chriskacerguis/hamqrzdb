#!/usr/bin/env python3
"""
FCC ULS Amateur Radio Callsign Data Processor
Processes HD.dat, EN.dat, and AM.dat files to generate JSON callsign records
"""
...
class ULSProcessor:
    """Process ULS amateur radio data files"""
    ...
    def parse_hd_file(self, file_path: str, filter_callsign: Optional[str] = None):
        ...
        with open(file_path, 'r', encoding='latin-1') as f:
            ...
            for row in reader:
                ...
                # HD fields: Record Type|Unique System Identifier|ULS File Number|
                # EBF Number|Call Sign|License Status|Radio Service Code|Grant Date|
                # Expired Date|Cancellation Date|Eligibility Rule Num|Reserved|
                # Alien|Alien Government|Trustee Call Sign|Trustee Indicator|
                # Trustee Name|Previous Call Sign|Previous Operator Class|
                # Trustee Name Continued
    ...
    def parse_en_file(self, file_path: str, filter_callsign: Optional[str] = None):
        ...
        with open(file_path, 'r', encoding='latin-1') as f:
            ...
            for row in reader:
                ...
                # EN fields: Record Type|Unique System Identifier|ULS File Number|
                # EBF Number|Call Sign|Entity Type|Licensee ID|Entity Name|
                # First Name|MI|Last Name|Suffix|Phone|Fax|Email|
                # Street Address|City|State|Zip Code|PO Box|
                # Attention Line|SGIN|FCC Registration Number (FRN)|
                # Applicant Type Code|Applicant Type Code Other|Status Code|
                # Status Date|3.7 GHz License Type
    ...
    def parse_am_file(self, file_path: str, filter_callsign: Optional[str] = None):
        ...
        with open(file_path, 'r', encoding='latin-1') as f:
            ...
            for row in reader:
                ...
                # AM fields: Record Type|Unique System Identifier|ULS File Number|
                # EBF Number|Call Sign|Operator Class|Group Code|
                # Region Code|Trustee Call Sign|Trustee Indicator|Physician Certification|
                # VE Signature|Systematic Call Sign Change|Vanity Call Sign Change|
                # Vanity Relationship|Previous Call Sign|Previous Operator Class|
                # Trustee Name
    ...
    def generate_json_record(self, callsign: str) -> Optional[Dict]:
        ...
        # For coordinates, we'd need location data which isn't in these files
        # You might need to add LA.dat (Location/Antenna) for lat/lon
        # For now, using empty strings
        lat = ""
        lon = ""
        grid = ""
    ...
    def write_json_file(self, callsign: str, record: Dict):
        ...
        # Create directory structure: output/K/J/5/KJ5DJC.json
        if len(callsign) >= 3:
...
def main():
    parser = argparse.ArgumentParser(
        description='Process FCC ULS amateur radio data and generate JSON files'
    )
    ...
    # Create processor
    processor = ULSProcessor(output_dir=args.output)
    ...
    if not hd_file.exists():
        print(f"HD.dat not found in extracted files")
        return 1
    if not en_file.exists():
        print(f"EN.dat not found in extracted files")
        return 1
    if not am_file.exists():
        print(f"AM.dat not found in extracted files")
        return 1
